...
* Low-cost. Coordinated demand response is essential to ensure quality of energy supply [jacome2019power](#reference), when none of large-scale storages, hydropower, nuclear power can be relied on for the promotion of power systems in underdeveloped area.
* Prosumers have time to react to price signals. The real-time incentives from spot markets are hard to catch, because not only the current signals can hardly reveal any information about future, but also prosumers need time to adapt their activities to fit traded quantities. [varian2017intermediate](#reference), [shugan2000advance](#reference)
